    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {items.map(item => (
        <div key={item.id} style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)",
          ...
          {item.url && <p style={{ fontSize: "0.8rem", marginBottom: "0.4rem" }}><a href={item.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--color-accent)", wordBreak: "break-all" }}>{item.url}</a></p>}
